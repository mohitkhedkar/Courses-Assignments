    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                // comments as needed
                float delta_x = point2X - point1X;

                float delta_y = point2Y - point1Y;

                float distance = (float)Math.Sqrt((Math.Pow(delta_x, 2) + (Math.Pow(delta_y, 2))));

                float angle = (float)Math.Atan2(delta_y, delta_x);

                float angleD = (float)((float)angle * (180 / Math.PI));

                Console.WriteLine(distance + " " + angleD);
